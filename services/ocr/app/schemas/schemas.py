...
class ReceiptItem(BaseModel):
    ...
    unit_price: float = Field(default=0, description="단가")
    quantity: float = Field(default=0, description="수량")
    amount: float = Field(default=0, description="총 금액")
...
class ReceiptResult(BaseModel):
    ...
    total_amount: float = Field(default=0, description="총 합계")
    discount_amount: float = Field(default=0, description="할인 금액")
    paid_amount: float = Field(default=0, description="실제 결제 금액")
