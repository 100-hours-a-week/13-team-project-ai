...
def _is_specific_restaurant_query(query: str) -> bool:
    ...
    if match:
        ...
        # "거기", "그곳" 등 지칭어면 특정 식당 검색이 아닌 멀티턴으로 유도해야 함
        if name in PRONOUNS:
            return False
        return True
    return False
...
def classify_query(query: str, history: list = None) -> str:
    ...
    # 2. 멀티턴 판단 (기록이 있을 때만)
    if history_len > 0:
        # 강력한 지칭어 ("거기", "그곳", "아까" 등)가 포함된 경우는 멀티턴 우선
        strong_context = ["거기", "그곳", "아까", "다시", "더", "말고", "다른", "아까 말한"]
        if any(k in query for k in strong_context) or bool(re.search(r"\d+\s*(명|인)", query)):
            return "multiturn"
        ...
        if any(k in query for k in facility_keywords) and not bool(re.search(r"[가-힣]{2,}(동|역|구|시)", query)):
            return "multiturn"
